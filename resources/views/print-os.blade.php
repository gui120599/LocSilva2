    ...
    <div class="p-1 max-w-6xl mx-auto ring-1 ring-gray-100/70">
        ...
        <header class="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-gray-100 pb-2 mb-2">
            <div class="space-y-1">
                <h2 class="font-extrabold text-gray-800">
                    Ordem de Serviço <span class="text-primary-600">{{ $os->numero }}</span>
                </h2>
                <p class="text-sm text-gray-500">
                    Aberta em: {{ $os->data_abertura?->format('d/m/Y H:i') ?? $os->created_at->format('d/m/Y H:i') }}
                </p>
                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-700">
                    Status: {{ $os->status->getLabel() }}
                </span>
                @if ($os->orcamento)
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-50 text-blue-700 ml-1">
                        Orçamento: {{ $os->orcamento->numero }}
                    </span>
                @endif
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-2">
            ...
            <div class="p-3 border border-gray-100 bg-gray-50/50 rounded-xl">
                ...
                <div class="space-y-1 text-[9px] text-gray-600">
                    <p><strong>Nome: </strong> {{ $os->nome_cliente ?? $os->cliente?->nome ?? '—' }}</p>
                    <p><strong>Telefone: </strong> {{ $os->telefone_cliente ?? $os->cliente?->telefone ?? '—' }}</p>
                    @if ($os->cliente)
                        <p><strong>CPF/CNPJ: </strong> {{ $os->cliente->cpf_cnpj ?? '—' }}</p>
                        <p><strong>Endereço: </strong> {{ $os->cliente->endereco ?? 'Não informado' }}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="p-3 border border-gray-100 bg-gray-50/50 rounded-xl">
                ...
                <div class="space-y-1 text-[9px] text-gray-600">
                    <p><strong>Descrição: </strong> {{ $os->veiculo_descricao ?? '—' }}</p>
                    <p><strong>Placa: </strong>
                        @if ($os->veiculo_placa)
                            <span class="font-mono bg-blue-100 text-blue-800 px-1.5 py-0.5 rounded">{{ $os->veiculo_placa }}</span>
                        @else
                            —
                        @endif
                    </p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-2">
            <div class="p-3 border border-gray-200 bg-white rounded-xl">
                <h3 class="text-base font-bold text-gray-700 mb-2 flex items-center">
                    <x-heroicon-s-calendar-days class="w-4 h-4 mr-2 text-blue-500" /> Datas
                </h3>
                <dl class="space-y-1 text-[10px]">
                    <div class="flex justify-between text-gray-700">
                        <dt>Abertura:</dt>
                        <dd>{{ $os->data_abertura?->format('d/m/Y H:i') ?? '—' }}</dd>
                    </div>
                    <div class="flex justify-between text-gray-700">
                        <dt>Previsão de Conclusão:</dt>
                        <dd>{{ $os->data_previsao_conclusao?->format('d/m/Y H:i') ?? 'Não definida' }}</dd>
                    </div>
                    <div class="flex justify-between text-gray-700">
                        <dt>Concluída em:</dt>
                        <dd>{{ $os->data_conclusao?->format('d/m/Y H:i') ?? 'Em aberto' }}</dd>
                    </div>
                </dl>
            </div>
            <div class="p-3 border border-gray-200 bg-white rounded-xl">
                <h3 class="text-base font-bold text-gray-700 mb-2 flex items-center">
                    <x-heroicon-s-wrench-screwdriver class="w-4 h-4 mr-2 text-yellow-500" /> Execução
                </h3>
                <dl class="space-y-1 text-[10px]">
                    <div class="flex justify-between text-gray-700">
                        <dt>Técnico Responsável:</dt>
                        <dd>{{ $os->tecnico?->name ?? 'Não atribuído' }}</dd>
                    </div>
                    <div class="flex justify-between text-gray-700">
                        <dt>Aberta por:</dt>
                        <dd>{{ $os->user?->name ?? '—' }}</dd>
        ...
        <div class="p-2 border border-gray-200 bg-primary-50/30 rounded-xl mb-2">
            ...
            <dl class="space-y-1 text-[10px] max-w-xs ml-auto">
                ...
                <div class="flex justify-between text-gray-700">
                    <dt>(+) Acréscimos:</dt>
                    <dd>R$ {{ number_format((float) ($os->valor_acrescimo ?? 0), 2, ',', '.') }}</dd>
                </div>
                <div class="flex justify-between text-gray-700">
                    <dt>(-) Descontos:</dt>
                    <dd>R$ {{ number_format((float) ($os->valor_desconto ?? 0), 2, ',', '.') }}</dd>
                </div>
                <div class="flex justify-between font-bold text-xs pt-1 border-t border-gray-200">
                    <dt class="text-gray-800">VALOR TOTAL:</dt>
                    <dd class="text-green-600">R$ {{ number_format((float) ($os->valor_total ?? 0), 2, ',', '.') }}</dd>
                </div>
                <div class="flex justify-between text-gray-700 pt-1">
                    <dt>(-) Total Pago:</dt>
                    <dd class="text-blue-700">R$ {{ number_format((float) ($os->valor_pago ?? 0), 2, ',', '.') }}</dd>
                </div>
                <div class="flex justify-between font-bold text-xs border-t border-gray-200 pt-1 {{ ($os->valor_saldo ?? 0) > 0 ? 'text-red-600' : 'text-green-600' }}">
                    <dt>SALDO RESTANTE:</dt>
                    <dd>R$ {{ number_format((float) ($os->valor_saldo ?? 0), 2, ',', '.') }}</dd>
                </div>
            </dl>
        </div>

        <!-- Pagamentos -->
        @if ($os->movimentos->count())
            <div class="p-2 border border-gray-200 bg-white rounded-xl mb-2">
                ...
                <ul class="space-y-1 text-[10px]">
                    @foreach ($os->movimentos as $pagamento)
                ...
                <div class="flex justify-between font-bold text-xs mt-2 pt-1 border-t border-gray-200">
                    <dt class="text-gray-700">Total Pago:</dt>
                    <dd class="text-green-700">
                        R$ {{ number_format($os->movimentos->sum('valor_total_movimento'), 2, ',', '.') }}
                    </dd>
                </div>
            </div>
        @endif
        ...
        @if ($os->observacoes)
            <div class="p-2 border border-gray-200 bg-white rounded-xl mb-2">
                ...
                <p class="text-[10px] text-gray-600 whitespace-pre-line">{{ $os->observacoes }}</p>
            </div>
        @endif

        <!-- Observações Técnicas -->
        @if ($os->observacoes_tecnicas)
            <div class="p-2 border border-gray-200 bg-yellow-50/40 rounded-xl mb-2">
                <h5 class="font-bold text-gray-700 mb-1 flex items-center">
                    <x-heroicon-s-wrench-screwdriver class="w-5 h-5 mr-2 text-yellow-600" /> Observações Técnicas
                </h5>
                <p class="text-[10px] text-gray-600 whitespace-pre-line">{{ $os->observacoes_tecnicas }}</p>
            </div>
        @endif
        ...
        <div class="grid grid-cols-2 gap-12 text-center mt-6">
            <div>
                ...
                <p class="font-semibold text-gray-800 text-sm">
                    {{ $os->nome_cliente ?? $os->cliente?->nome ?? 'Cliente' }}
                </p>
